...
d[1:n] = randn(n)
y = copy(d[1:n])
GeneralizedSylvester.solvi_real_eliminate!(index,n,nd,1:nd,depth-1,r,t,s,d,ws)
...
d[nd+(1:nd)] = randn(nd)
y = copy(d[nd+(1:nd)])
GeneralizedSylvester.solvi_real_eliminate!(index,n,nd,nd+(1:nd),depth-1,r,t,s,d,ws)
...
t2 = t*t
s2 = s*s
GeneralizedSylvester.solve1!(r,depth,t,t2,s,s2,d,ws)
...
d = randn(2*n^(depth+1))
d_orig = copy(d)
GeneralizedSylvester.transformation1(a,b1,b2,depth,tt,ss,d,ws)
...
r2 = 0.8
td = []
GeneralizedSylvester.solviip_real_eliminate!(index,n,nd,1:nd,depth-1,r1,r2,t,t2,s,s2,d,ws)
...
d = randn(2*nd)
d_orig = copy(d)
GeneralizedSylvester.transform2(r1, r2, gamma, delta1, delta2, nd, depth, t, t2, s, s2, d, ws)
...
r2 = 0.8
td = []
GeneralizedSylvester.solviip_complex_eliminate!(index,n,nd,1:nd,depth-1,r1,r2,t,t2,s,s2,d,ws)
...
td = similar(d)
GeneralizedSylvester.solviip(alpha,beta,depth,t,t2,s,s2,d,ws)
...
r2 = sqrt(-beta1*beta2)
td = similar(d)
GeneralizedSylvester.solviip(r1,r2,depth,t,t2,s,s2,d,ws)
...
println("begin test")
GeneralizedSylvester.solver!(t,s,d,order,ws)
d_target = (eye(n^(order+1)) + kron(s',t))\d_orig
#d_target = (eye(n^(order+1)) + kron(kron(s',s'),t))\d_orig
@test d ≈ d_target
...
GeneralizedSylvester.generalized_sylvester_solver!(a,b,c,vec(d),2,ws)
...
function test1(m, n, order)
    ...
    ws = EyePlusAtKronBWS(m, m, n, order)
